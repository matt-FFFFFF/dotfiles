...
- **Appearance mode**: Driven by the `appearance` key in `colors.toml`, this sets `defaults write -g AppleInterfaceStyle Dark` (or deletes the key for light mode) and broadcasts `AppleInterfaceThemeChangedNotification` via `osascript` so already-running apps repoll appearance live.
- **Mouse pointer (cursor) fill + outline**: Fill driven by the `cursor` key in `colors.toml`. Written via `defaults write com.apple.universalaccess cursorFill -dict red/green/blue/alpha -float ...` (must go through cfprefsd — direct plist mutation bypasses the distributed notification and apps don't pick up the change). Outline is auto-picked by WCAG relative luminance of the fill (`0.2126·R + 0.7152·G + 0.0722·B`): light fill (luminance > 0.5) gets a black outline, dark fill gets white. Already-running apps refresh their cached cursor on the next focus event (click, app switch); they won't update mid-air without one. The **Dock** is the stubborn exception — its cursor is cached at the WindowServer layer and survives `killall Dock`; a screen lock + unlock (`Ctrl+Cmd+Q`) is the cheapest way to flush it short of a logout.
